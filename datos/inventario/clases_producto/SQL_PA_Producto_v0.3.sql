...
DELIMITER $$
	...
	BEGIN
		SELECT * FROM tb_producto WHERE intIdProducto = 
 		LIMIT _x,_y;
    	END 
$$
DELIMITER ;
...
DELIMITER $$
	...
	BEGIN
		...
		WHERE 
		intIdProducto LIKE CONCAT(_elemento,'%') OR 
		nvchDescripv LIKE CONCAT(_elemento,'%') OR 
		dcmPrecio LIKE CONCAT(_elemento,'%') OR 
		intCantidad LIKE CONCAT(_elemento,'%') OR 
		nvchDescripcion LIKE CONCAT(_elemento,'%');
    	END 
...
DROP PROCEDURE IF EXISTS CONSULTARIMAGENPRODUCTO;
DELIMITER $$
	CREATE PROCEDURE CONSULTARIMAGENPRODUCTO(
	IN _intIdProducto INT,
    )
	BEGIN
		SELECT nvchDireccionImg FROM tb_producto
		WHERE 
		intIdProducto = _intIdProducto;
    	END 
$$
