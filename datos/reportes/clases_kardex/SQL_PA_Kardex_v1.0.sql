...
DELIMITER $$
	...
	BEGIN
		...
		LEFT JOIN tb_detalle_comprobante DCR ON CR.intIdComprobante = DCR.intIdComprobante
		WHERE DCR.intIdProducto = _intIdProducto AND CR.intIdTipoComprobante != 3 AND CR.intIdTipoComprobante != 7
		ORDER BY DCR.dtmFechaRealizada,CR.intIdComprobante ASC) AS Comprobantes GROUP BY Comprobantes.FechaMovimiento,Comprobantes.Iden)
		AS KardexGeneral ORDER BY KardexGeneral.FechaMovimiento,KardexGeneral.Iden DESC;
