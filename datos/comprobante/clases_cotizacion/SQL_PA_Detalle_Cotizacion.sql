...
DELIMITER $$
	...
	BEGIN
		...
		(intIdCotizacion,intIdTipoVenta,dtmFechaRealizada,intIdProducto,nvchCodigo,nvchDescripcion,dcmPrecio,dcmDescuento,
			dcmPrecioUnitario,intCantidad,dcmTotal)
		VALUES
		(_intIdCotizacion,_intIdTipoVenta,_dtmFechaRealizada,_intIdProducto,_nvchCodigo,_nvchDescripcion,_dcmPrecio,_dcmDescuento,
...
DROP PROCEDURE IF EXISTS MOSTRARDETALLECOTIZACIONSERVICIO;
DELIMITER $$
	CREATE PROCEDURE MOSTRARDETALLECOTIZACIONSERVICIO(
    	IN _intIdCotizacion INT
    )
	BEGIN
		SELECT DCT.* FROM tb_detalle_cotizacion DCT
		WHERE
		DCT.intIdCotizacion = _intIdCotizacion;
    END 
$$
DELIMITER ;
